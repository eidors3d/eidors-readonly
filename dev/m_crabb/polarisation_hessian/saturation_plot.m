...
%Conductivity values
cond_vals = logspace(0,.1,25); %0.1 is approx 1.25 contrast
...
figure; semilogx(cond_vals,norm_vi_diff,'g*');
hold on; semilogx(cond_vals,norm_vi_diff_lin,'r*');
hold on; semilogx(cond_vals,norm_vi_diff_quad,'b*');
...
figure; semilogx(cond_vals,abs(norm_vi_diff-norm_vi_diff_lin),'r*');
hold on; semilogx(cond_vals,abs(norm_vi_diff-norm_vi_diff_quad),'b*');
legend('Linear-Nonlinear','Quadratic-Nonlinear')
ylabel('||V_nonlin-V1||-||V_approx-V1||'); 
xlabel('Pertubation in ith pixel');
