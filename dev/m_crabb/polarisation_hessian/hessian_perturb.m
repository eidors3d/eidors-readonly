function H= hessian_perturb( fwd_model, img)
% JACOBIAN_PERTURB: H= hessian_perturb( fwd_model, img)
% Calculate Hessian Matrix, based on small perturbations
%   in the jacobian. This will tend to be slow, but
%   should be best used to 'sanity check' other code
%
% H         = Hessian matrix
% fwd_model = forward model
% fwd_model.hessian_perturb.delta   - delta perturbation to use
% img = image background for hessian calc

%if ischar(fwd_model) && strcmp(fwd_model,'UNIT_TEST'); do_unit_test; return;end

if isfield(fwd_model,'hessian_perturb')
   delta = fwd_model.hessian_perturb.delta;
else
   delta= 1e-6; 
end
...
%   end
   ...
   Hcol= perturb(img, 1, delta, J0, curprms);

   H= zeros(size(Hcol,1),size(Hcol,2), n_elem);
   H(:,:,1)= Hcol;

   for i=2:n_elem
       i
     H(:,:,i)= perturb(img, i, delta, J0, curprms);
   end
end


function Jcol= perturb( img, i, delta, J0, curprms)
   if ~isempty(curprms)
      img.(curprms).elem_data(i)= img.(curprms).elem_data(i) + delta;
   else
      img.elem_data(i)= img.elem_data(i) + delta;
   end
   Ji= jacobian_adjoint( img );
   Jcol = (1/delta) * (Ji - J0);
