function[data,nodeunknowns] = mc_fwd_solve_CEM(img)
...
groundnode=mdl.gnd_node; At=mdl.solver.At;
...
for ii=1:nstims
    ...
    for i=1:nelecs
        rhsdata(i+nnodes,ii)=curnode(i);
    end
end
%Eliminate the ground node equation from data
rhsdata(groundnode,:)=[];

%Solve all the linear systems using backslash/forward_solver
nodetemp=forward_solver(At,rhsdata);
%nodetemp=At\rhsdata;
...
%Calculate electrode voltages for CEM (matrix nelecs*nstim)
velec=zeros(nelecs,nstims);
for i=1:nelecs
    %This is the last entries in nodeunknowns
    velec(i,:)=nodeunknowns(nnodes+i,:);
end
