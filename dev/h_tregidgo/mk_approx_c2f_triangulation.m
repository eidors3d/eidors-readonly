...
function tsn= search_fm_pts_in_cm(cm, fm_pts, z_depth);
    ...
    if(exist('OCTAVE_VERSION') || verLessThan('matlab','8.5.0'))
        ...
        % 2015 (8.5.0). pointLocation is faster and more accurate than
        % tsearchn.
        temp_triangulation=triangulation(cm.elems,cm.nodes(:,dims));
        tsn(not_oor)= pointLocation(temp_triangulation,fm_pts(not_oor,dims));
    end
