function [fmdl,mat_idx] = ng_mk_cyl_models_keepfiles(cyl_shape, elec_pos, ...
                  elec_shape, varargin);

% Modified function to accept an optional additional argument.  If this
% argument is passed then the (usually temporary) mesh files are not
% deleted.  Argument is passed before extra_ng_code (if extra_ng_code is
% specified).  Currently the argument value is ignored, only its
% existence matters.
% Scott Webster
% November 22, 2012

% NG_MAKE_CYL_MODELS_KEEPFILES: create cylindrical models using netgen,
% optionally retain mesh files
%[fmdl,mat_idx] = ng_mk_cyl_models(cyl_shape, elec_pos, ...
%                 elec_shape, [keepfiles], [extra_ng_code]);
% INPUT:
...
% by default, remove temporary files
keepfiles=0;

% if we have optional arguments
if nargin > 3
  % extra netgen code is provided in a cell array, if this is the first
  % optional argument then parse that and we're done, delete temp files
  % as usual
  if iscell(varargin{1})
    keepfiles = 0;
    extra_ng_code=varargin{1};
  % if the first optional argument is not a cell array then it means the
  % user has passed something for the keepfiles argument, assume they
  % want to retain the files, could be improved to allow a boolean toggle
  else
    keepfiles = 1;
    % if at this point there is another optional argument, assume it is
    % extra netgen code
    if length(varargin) > 1
      extra_ng_code=varargin{2};
    else
      extra_ng_code = {'',''};
    end
  end
% if there are no optional arguments assign empty value to extra_ng_code
else
  extra_ng_code = {'',''};
end
...
if isempty(fmdl);
   fmdl = mk_cyl_model( cyl_shape, elec_pos, elec_shape, keepfiles, extra_ng_code );
...
function [fmdl_mat_idx] = mk_cyl_model( cyl_shape, elec_pos, elec_shape, keepfiles, extra_ng_code );
   ...
   % keep temp files if requested by user
   if ~keepfiles
