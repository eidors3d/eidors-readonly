...
function unit_test_simdata
   ...
   imdl.fwd_model.solve = @fwd_solve_apparent_resistivity;
   imdl.fwd_model.jacobian = @jacobian_apparent_resistivity;
...
%    I= imdl.parameters.normalisation;
   figure; plot((dd.meas-vCG))
   figure; hist((dd.meas-vCG),50)

   show_pseudosection( fmdl, dd.meas, 'profile')
   show_pseudosection( fmdl, vCG, 'profile')
...
function [fmdl, cmdl] = unit_test_models
   shape_str = ['solid top    = plane(0,0,0;0,1,0);\n' ...
                'solid mainobj= top and orthobrick(-100,-200,-100;410,10,100) -maxh=40.0;\n'];
