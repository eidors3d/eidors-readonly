...
for i = 1:opt.max_iter
  ...
  RDx = hp2RtR*(img0.inv_params - tmp.inv_params);
...
function val = GN_objective_function(data0, data, img0, img, opt)
   ...
   if ~isfield(img0, 'inv_params'), img0 = params_mapper(img0); end
   if ~isfield(img, 'inv_params'), img = params_mapper(img); end
...
function img = initial_estimate( imdl, data )
   ...
   if isfield(img.fwd_model,'coarse2fine');
      ...
      % TODO: this assumes that inv_params is elem_data...
      
      % TODO: the two cases are very different. c2f case should match other
      nc = size(img.fwd_model.coarse2fine,2);
      img.inv_params = mean(img.inv_params)*ones(nc,1)*pf(1);
      
      warning('c2f needs work');
   else
      img.inv_params = img.inv_params*pf(1);
   end
