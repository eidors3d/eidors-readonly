...
if isempty(param_save) || all(fwd_model.dg_fwd_solve.compute_CCandSS=='y')
...
for i=1:n_elec
    ...
    if length(elec_nodes) == 1 % case for a point electrode
       N2E(i,elec_nodes) = 1;
    else                       % case for an electrode distributed on a surface
