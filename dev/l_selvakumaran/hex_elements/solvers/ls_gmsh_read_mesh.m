function[srf,vtx,fc,bc,simp,edg,mat_ind,phys_names] = ls_gmsh_read_mesh(filename)
...
% volume (simp) and edges (edg) for hex/quad meshes
%
...
if length( unique( nodes(:,4) ) ) > 1; 
    ...
    quad = find(arrayfun(@(x)x.type==3,elements));
    % Type 4: 4-node tetrahedron
    hex = arrayfun(@(x)x.type==5,elements);
    simp = cat(1,elements(hex).simp);
    srf = cat(1,elements(quad).simp);
else
    vtx = nodes(:,2:3);
    quad = find(arrayfun(@(x)x.type==3,elements));
    simp = cat(1,elements(quad).simp);
    srf = [];
end
