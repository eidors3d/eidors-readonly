function [fwd_mdl, mat_indices]= ...
             ls_gmsh_mk_fwd_model( vol_filename, name, eprefix,stim_pattern, z_contact)
% GMSH_MK_FWD_MODEL: create a fwd_model object from a Gmsh file with
% quad/hex elements
% [fwd_mdl, mat_indices]= ...
...
% Model Geometry
[srf,vtx,fc,bc,simp,edg,mat_ind,phys_names] = ls_gmsh_read_mesh(vol_filename);
...
if isempty(srf);
   fwd_mdl.boundary = ls_find_boundary(fwd_mdl);
end
...
function fwd_mdl= construct_fwd_model(srf,vtx,simp,bc, name, ...
    ...
    mdl.solve=          'ls_fwd_solve_1st_order';
