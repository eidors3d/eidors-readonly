...
function opt = parse_options(opt,fmdl,imdl);
    ...
    % if we do have a target plane, modify minnode to around that level
    mdl_h = opt.maxnode(3) - opt.minnode(3);
    nn = fmdl.nodes(:,3) > opt.target_plane - 0.1*mdl_h  &  ...
         fmdl.nodes(:,3) < opt.target_plane + 0.1*mdl_h ;
    opt.minnode(1:2) = min(fmdl.nodes(nn,1:2));
    opt.maxnode(1:2) = max(fmdl.nodes(nn,1:2));
