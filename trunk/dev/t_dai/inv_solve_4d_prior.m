function img= inv_solve_4d_prior( inv_model, data1, data2)
% inv_solve_4d_prior-- inverse solver to account for temporal
% and 3D spatial correlation
% img= inv_solve_4d_prior( inv_model, data1, data2)
...
one_step_inv = eidors_obj('get-cache', inv_model, 'inv_solve_4d_prior');
if ~isempty(one_step_inv)
    eidors_msg('inv_solve_4d_prior: using cached value', 2);
else
    ...
    eidors_obj('set-cache', inv_model, 'inv_solve_4d_prior', one_step_inv);
...
img.name= 'solved by inv_solve_4d_prior';
img.elem_data = sol;
