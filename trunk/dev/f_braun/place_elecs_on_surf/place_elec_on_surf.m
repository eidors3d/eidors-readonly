...
mdl.is_electrode = false(size(mdl.elems, 1), 1);
...
% remove faces which are located on other electrodes
% this avoids errors when placing electrodes very close to each other
if any(mdl.is_electrode(N));
   eidors_msg('Removing faces potentially located on other electrodes!', 3);
   N(mdl.is_electrode(N)) = [];
   
   % if more then one connected component exists, 
   % -> keep only the one containing the electrode center face
   ConnComps = manifold_patches(mdl.elems(N,:));
   OnElectrodePatch = (ConnComps == ConnComps(N == fc));
   N(~OnElectrodePatch) = [];
end
fcs = N;
...
jnk.boundary = jnk.elems;
jnk.is_electrode = [false(size(els, 1), 1); true(size(E,1) + size(elecs.elems, 1), 1)];
if do_debug
   show_fem(jnk);
end
...
big.normals(N,:) = [];
big.face_centre(N,:) = [];
assert(all(~big.is_electrode(N,:)), 'we cannot remove faces of existing electrodes.');
big.is_electrode(N,:) = [];

big.elems = big.boundary;

big.mat_idx = {1:size(big.elems,1)};
joint = merge_meshes(big,jnk,0.001);
joint.boundary = joint.elems;
joint.faces = joint.boundary;
joint.is_electrode = false(size(joint.elems,1),1);
joint.is_electrode(joint.mat_idx{1}) = big.is_electrode;
joint.is_electrode(joint.mat_idx{2}) = jnk.is_electrode;
opt.normals = true;
...
Ne = mdl.normals(fc,:);
Pe = elecs.pos;
% prior to calculation: remove nodes which are too far away (out of bounding sphere)
InBounds = @(xyz, ctr, R) ((xyz(:,1)-ctr(1)).^2 + (xyz(:,2)-ctr(2)).^2 + ...
                           (xyz(:,3)-ctr(3)).^2 < R^2);
r_bounds = max(elecs.dims) * 4;   % here we're pretty pessimistic, just to be on the safe side
% nodes_close_to_elec = find(InBounds(mdl.nodes(nodes,:), mdl.face_centre(fc,:), r_bounds));
nodes_close_to_elec = find(InBounds(mdl.nodes(nodes,:), Pe, r_bounds));
% pre-allocate for speedup reasons
PN = inf * ones(length(nodes), size(mdl.nodes,2));
for i = 1:length(nodes_close_to_elec)
   P = mdl.nodes(nodes(nodes_close_to_elec(i)),:);
   PN(nodes_close_to_elec(i),:) = P + dot(Pe - P, Ne) * Ne;
end
...
if use_elec
   ...
   rng = emax-emin;
   rng = max(elecs.maxh*[1 1 1], rng);  % avoid a too "thin" range
   emin = emin - 0.1*rng;
   emax = emax + 0.1*rng;
...
% Credits: Alec Jacobson, No licence given, assume public domain
