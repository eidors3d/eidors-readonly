...
%% This function was used for TA052, because there the sides are better formed than the front and back
...
for i=1:size(y_along,1)
    newline_y = inf*ones(size(x_along,1)+1,3);
    newline_y(1,:) = y_along(i,:);
    for j=1:size(x_along,1)
        
        dist_y=(sum((sel-repmat(newline_y(j,:),size(sel,1),1)).^2,2)).^0.5;
        dist_x=(sum((sel-repmat(x_along(j,:),size(sel,1),1)).^2,2)).^0.5;
        
        p=find(abs(dist_y-point_distance)<eps & abs(dist_x-point_distance)<eps); % find the intersection of the two stripes
    ...
    x_along = newline_y(2:end,:);
    
    % write the new line into the coordinate matrix
    if x_dir>0
        %coordinates(origin_m+1:end,origin_n + x_dir*3*i:origin_n + x_dir*3*i + 2) = newline_y(2:end,:);
        coordinates(origin_m+y_dir*i, origin_n+3:end) = reshape(newline_y(2:end,:)',1,size(newline_y(2:end,:),1)*size(newline_y(2:end,:),2));
    else
        %coordinates(1:origin_m-1,origin_n + x_dir*3*i:origin_n + x_dir*3*i + 2) = flipud(newline_y(2:end,:));
        coordinates(origin_m+y_dir*i, 1:origin_n-1) = reshape(flipud(newline_y(2:end,:))',1,size(newline_y(2:end,:),1)*size(newline_y(2:end,:),2));
    end
end
