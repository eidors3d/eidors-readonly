...
for rad=[0.1]
 ...
 %   figure; show_fem(img);
    ...
    %Phessian Jacobian
    img_b.fwd_solve.get_all_meas=1;
    d_k = fwd_solve(img_b); % doesn't contribute to cts as cached value here
    u0= d_k.volt;
    DU0 = calc_grad_potential(img_b, u0);
    [~,~,~,J_phess] = calc_phessian_obj(img_b.fwd_model,img_b,DU0,d_k.meas,'disc');
    ...
    img_b.fwd_model.approx_type='tri3';
    ...
    %% POLARISATION HESSIAN DIAGONAL HERE??
