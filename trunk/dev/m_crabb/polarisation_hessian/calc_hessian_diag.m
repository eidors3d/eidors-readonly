function H_diag = calc_hessian_diag(fwd_model,img,elem_list)
%Find the diagonal of Hessian associated with an image (and forward model)
...
H_diag = eidors_obj('get-cache', cache_obj, 'hessian_diag');
if ~isempty(H_diag)
            eidors_msg('hessian_diag: using cached value', 3);        
        return
end


[H_diag]= mc_calc_hessian_diag(fwd_model,img,elem_list);

eidors_obj('set-cache', cache_obj, 'hessian_diag',H_diag);
eidors_msg('hessian_diag: setting cached value', 3);

end

function H = mc_calc_hessian_diag(fwd_model,img,elem_list)
...
for idxkk=1:length(elem_list)
        ...
        D2E(:,:,idxkk,idxkk) = zi2E(:,idx)*dA_dSk2E(idx,:)*dA_dSk(idx,idx)*nodeunknownsfwd(idx,:) + ...
                               zi2E(:,idx)*dA_dSk2E(idx,:)*dA_dSk(idx,idx)*nodeunknownsfwd(idx,:);
    fprintf(1,'Diagonal Hessian completed for element %i of %i\n',idxkk,length(elem_list));                          
end  
