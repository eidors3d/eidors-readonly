function Reg= prior_electrode_movement( inv_model );
...
%% NOTE WITH ELECTRODE MOVEMENT NOW WE HAVE 1DOF PER ELECTRODE THIS IS
%% EXACTLY THE SAME AS PRIOR MOVEMENT EXPECT CALL FUNCTIONS WITH (dim-1)

% calc movement portion
try
   fname = func2str(inv_model.prior_movement.RegM.func);
   if(strcmp(fname,'tikhonov_movement_image_prior'))
       RegM = tikhonov_movement_image_prior(pp.n_dims-1,pp.n_elec);
   elseif(strcmp(fname,'laplace_movement_image_prior'))
       RegM=laplace_movement_image_prior(pp.n_dims-1,pp.n_elec);
   end
catch
   RegM = laplace_movement_image_prior( pp.n_dims-1, pp.n_elec );
end
