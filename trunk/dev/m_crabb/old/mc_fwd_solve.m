...
if(size(elecstruc(1).nodes,2)==1 && size(elecstruc(1).nodes,1)==1) %POINT
    ...
    rhsdata=zeros(nnodes,nstims); nodeunknowns=zeros(nnodes,nstims); 
else
    for i=1:nelecs
    ...
    rhsdata=zeros(nnodes+nelecs,nstims); nodeunknowns=zeros(nnodes+nelecs,nstims); 
end
...
%INITIALISE - Electrode index vectors, ground nodes and assign current
...
%Eliminate ground node equation from data
rhsdata(groundnode,:)=[];
...
%tic; 
nodetemp=forward_solver(At,rhsdata);%nodetemp=At\rhsdata; 
...
%POST-PROCESS - Find electrode voltages and store in matrix
...
%RETURN - Return the electrode voltages in data structure
