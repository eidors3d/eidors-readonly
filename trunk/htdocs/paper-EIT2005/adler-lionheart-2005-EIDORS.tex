...
% $Id: adler-lionheart-2005-EIDORS.tex,v 1.34 2005-11-07 05:37:29 aadler Exp $
...
\caption{\small
The 3D demonstration FEM provided with the EIDORS3D
...
has a weighting of $D+1$, where $D$ is the model dimention,
...
coarse FEM mesh of only xx elements with 32 rectangular
...
netgen. The electodes were created by the intesection
...
of $10^{-8}$ appears in \fref{fig:3fingers1}. It is
displayed using the VTK visualisation program MayaVi
...
Earlier versions of EIDORS provided 2D image
...
has a very different focus. Rather than
...
\begin{itemize}
  ...
  into which many FEM models, image algorithms, and
  ...
  For numerical difficulties, we provide several well used independant
  ...
  hand, we are not able to address by software. Instead we
...
Open source software like EIDORS has could
potentially dramatically facilitate
research and development in medical imaging fields like EIT.
It could assist in the development on new works
based on combinations of previous approaches, and
could allow correct comparison of new approaches
to older ones. 

We make the following additional observations related to 
``cheating'' with inverse problems.
We intentionally take a
lighthearted tone (ie\. using terms like ``cheating'',
and ``happy transform'') because we do not want to
...
concern, in the sense that there existance is obvious.
Such cases can be described by the aphorism:
``Garbage in; garbage out''.
The worrysome errors are those that introduce the possibility
of false image interpretation, since the quality of the
produced image does not obviously call it into question.
The example of a trasformation of a sad to happy face 
...
that can be misinterpreted in this way. We would like
to introduce the following edict: 
``Don't whitewash our garbage!''.


In conclusiong, we have described the new
