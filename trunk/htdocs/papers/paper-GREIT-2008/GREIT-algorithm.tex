...
We define SNR~=~%
$\frac{
   \mbox{\mathrm{mean} \em signal}
      }{
   \mbox{\mathrm{std} \em noise}
     }
$ 
...
define the GREIT reconstruction matrix
...
The GREIT
reconstruction matrix $\RB$ (later referred to as $\RB_{GR}$)
which best fits the requirements
may be expressed as minimization of an error $\epsilon^2$
\begin{equation}
\label{GREIT_norm}
\epsilon^2 = \sum_k \| \xT^{(k)} - \RB \yB^{(k)} \|^2_{\WB^{(k)}}
\end{equation}
%\epsilon^2 = \sum_k \| \xT)} - \RB_{GR} \yB^{(k)} \|^2_{\left(\WB^{(k)}\right)^2}
%   = \sum_k \| (\WB^{(k)})^2 (\xT^{(k)} - \RB \yB^{(k)} ) \|^2
where the sum $k$ is over all training measurement and noise
samples, and
$\WB^{(k)} = \mathrm{diag}( \wB^{(k)})$, is a diagonal matrix representing
...
-2 \sum_k (\xH^{(k)})^T \WB^{(k)} [\TG]_{i,j} \yB^{(k)}
+2 \sum_k (\yB^{(k)})^T [\TG]_{i,j} \WB^{(k)} \RB \yB_k
\end{equation}
where $[\TG]_{i,j}$ has a value of $1$ at $(i,j)$
but is zero elsewhere. Based on this expression
\begin{eqnarray}
  \sum_k (\xH^{(k)})^T \WB^{(k)} [\TG]_{i,j} \yB^{(k)}
 &=&
  \sum_k (\yB^{(k)})^T [\TG]_{i,j} \WB^{(k)} \RB \yB^{(k)}
\nonumber
\\
  \sum_k [\yB^{(k)}]_j [\wB^{(k)}]_i [\xT^{(k)}]_i 
 &=&
  \sum_l [\RB]_{i,l} \sum_k [\yB^{(k)}]_l [\wB^{(k)}]_i [\yB^{(k)}]_j 
\end{eqnarray}

This expression may be represented as a matrix equation
by defining matrices $\AB$ and $\BB$, such that
\begin{equation}
\AB = \sum_k \WB^{(k)} \xT^{(k)} (\yB^{(k)})^T 
\end{equation}
and $\BB$ is the vertical concatenation of $n_N$
($n_M \times n_M$) block matrices $\BB_l$, where
\begin{equation}
\BB_l = \sum_k [\wB^{(k)}]_l \yB^{(k)} (\yB^{(k)})^T 
\end{equation}
