function show_current( img, volt )
...
elemcur = zeros(size(fmdl.elems,1)+1,size(fmdl.nodes,2));
vv =  vh.volt(:,1);
del = [1,-1,0;0,1,-1;-1,0,1];
del2 = del'*del;
for i=1:size(elemcur,1)-1;
  idx = img.fwd_model.elems(i,:);
  nod = img.fwd_model.nodes(idx,:);
  elemcur(i+1,:) = vv(idx)'*del2*nod;
end

[x,y] = grid_the_space( img.fmdl);
...
function do_unit_test
   ...
   img.fwd_solve.mdl_slice_mapper.npx = 64;
   img.fwd_solve.mdl_slice_mapper.npy = 64;
   show_current(img);

xx = linspace(0,1,64);
fmdl.mdl_slice_mapper.x_pts = xx;
fmdl.mdl_slice_mapper.y_pts = xx;
fmdl.mdl_slice_mapper.level = [inf,inf,0];


   img.fwd_solve.get_all_meas = 1; vh = fwd_solve(img);
