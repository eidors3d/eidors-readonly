...
\newcommand{\XB}{\mbox{$\mathbf{X}$}}
\newcommand{\YB}{\mbox{$\mathbf{Y}$}}
% Use boldsymbol is using amssymb
...
Given a vector of EIT difference of normalized difference
data, $\yB$ (length $N_E N_V$), we calculate a 
reconstructed EIT image $\xH = \RB \yB$ based
...
contains all image pixels $[\xH]_i$ for
which $[\xH]_i \geq \mathrm{max}~\xH$. For targets
with a conductivity decrease the definition
of the $\frac{1}{4}$--amplitude set is inverted.
The centre of gravity (CoG) of for $\xH$ and $\xH_q$ are
...
a small spherical target with $\sigma_t/\sigma_r \approx 1$
...
constant amplitude repsponse, the same volume of air in different parts
...
target position, $r_t$, and the CoG of $\xH_q$, we define:
\begin{equation}
\mathrm{PE} = r_q - r_t
\end{equation}
...
\end{equation}
where $A_{\textrm{medium}}$ is the area (in pixels) of
the reconstructed medium. The square root is used to
...
\hspace{5mm}
{\em Desired behaviour:}
RES should be uniform and small, in order to accurately 
...
to distinguish nearby targets. Since EIT is understood
...
shows an arc of overshoot around the image centre.
Overshoot is typical of linear filters; for second-order
systems the effect may also be called ``ringing''.
OVS measures the ratio of image amplitude of
...
\hspace{5mm}
{\em Desired behaviour:}
OVS should be low and uniform. Overshoot may
easily result in incorrect interpretation. For
example, the negative overshoot between non-conductive
lungs will produce a conductive pattern which 
may be misinterpreted as a heart.
We consider OVS to be the third most important figure 
of merit.
...
illustrated in Fig. \ref{fig:noise_fig}; this
definition is based on that of Adler and Guardo (1996).
\begin{equation}
\mathrm{OVS} = \sum_{k\not\in C \& [\xH]_k < 0} [\xH]_k / 
               \sum_{k\in C}                    [\xH]_k 
\end{equation}
For regularized algorithms, the value of NF may be controlled
a hyperparameter. For GREIT, NF is controlled by the weighting
...
way to choose NF. Instead, we note that Sheffield Backprojection
has generally satisfactory noise performance. This
algorithm has NF$=0.5$ at the medium centre; we recommend
this value for GREIT algorithms.
...
of data artefacts can be more readily be identified. Finally,
...
For lung images, we may calculate $\sigma_{\xB}^2$ from either
...
each pixel in $\xT^{(k)}$. The weighing allows ``tuning''
...
corresponds to the selection strategy for
the training data.
...
a GREIT reconstruction algorithm with formulated
...
using the linear asumption. This formulation 
