...
\nonumber \\
&=& \sum_k \sum_i \Bigg( 
   [\xH^{(k)}]_{i}^2 [\wB^{(k)}]_{i}^2
-2 [\xH^{(k)}]_{i}   [\wB^{(k)}]_{i}^2  \Big( \sum_j \RB_{ij} [\yB^{(k)}]_{j} \Big)
\nonumber \\
& &
+                    [\wB^{(k)}]_{i}^2  \Big( \sum_j \RB_{ij} [\yB^{(k)}]_{j} \Big)^2
\Bigg)
\end{eqnarray}
...
\end{eqnarray}
where matrices $\AB \in \mathbb{R}^{n_N \times n_M}$
and            $\BB \in \mathbb{R}^{n_N \times n_M \times n_M}$
are defined as
$\AB = 
   \sum_k [\xH^{(k)}]_{i}  [\wB^{(k)}]_i^2 [\yB^{(k)}]_j $
and 
...
since the best selection of weights has not been defined.
