...
rapid, real-time imaging. The GN inverse problem seeks to
...
underdetermined since $n_N > n_M$. This problem is commonly solved
using regularization techniques (eg. Cheney et al 1990,
 Adler and Lionheart, 2006) in order to
 calculate a conductivity change
 estimate, $\xH$, which is both
faithful to the measurements, $\yB$, and to
{\em a priori} constraints on a ``reasonable'' image.


Typically,  $\SG_n$ and $\SG_x$ are not calculated directly.
Instead, their inverses,
 $\WB = \sigma_n^2\SG_n^{-1}$
 and
 $\RB = \sigma_x^2\SG_x^{-1}$,
are heuristically determined from {\em a priori} considerations.
Here $\sigma_n$ is the average measurement noise amplitude and
$\sigma_x$ is the {\em a priori} amplitude of conductivity change.
$\WB$ models the measurement accuracy. For uncorrelated noise,
each diagonal element is proportional to the signal to noise
ratio. For difference EIT with identical channels, $\WB=\IB$. The
regularization matrix $\RB$ may be understood to model the
``unlikelihood" of image elements.

By solving (\ref{IM}), a linearized, one-step inverse solution is
obtained as
\begin{equation}\label{LMS_solution}
\xH=\left(\JB^T\frac{1}{\sigma_n^2}\WB\JB+\frac{1}{\sigma_x^2}\RB\right)^{-1}\JB^T\frac{1}{\sigma_n^2}\WB\yB
\end{equation}
We define the hyperparameter $\lambda=\sigma_n/\sigma_x$,
 and rewrite (\ref{LMS_solution})
as
\begin{equation}\label{GN_solution}
\xH=\left(\JB^T\WB\JB+\lambda^2\RB\right)^{-1}\JB^T\WB\yB=\BB\yB
\end{equation}
where $\BB=\left(\JB^T\WB\JB+\lambda^2\RB\right)^{-1}\JB^T\WB$
...
expected magnitude, $\RB$ becomes an identity matrix $\IB$ and
...
boundary image elements. Instead, $\RB$ may be scaled with the
sensitivity of each element, so that each diagonal element $i$ of
$\RB$ is $\left[ \RB \right]_{i,i} = \left[ \JB^T \JB
...
image regions, $\RB$ may be set to the discrete Laplacian filter
(Vauhkonen, 1998b) or a discrete high pass Gaussian filter (Adler
and Guardo, 1996).

In this paper, the NOSER prior is used for calculating the matrix
$\RB$ with $p=0.5$ in all tested algorithms, except for the Kalman filtering.
Because it is diagonal, $\RB$ can be
...
accuracy and stability, we want to decrease the size of the matrix
to be inverted.  Thus, we rewrite the matrix $\BB$ using the {\em
data form} as:
\begin{eqnarray}\label{opti_sol}
=\left(\JB^T\WB\JB+\lambda^2\RB\right)^{-1}\JB^T\WB
\left[
    \left(
      \JB\frac{1}{\lambda^2}\PB\JB^T+\VB
   \right)
    \left(
      \JB\frac{1}{\lambda^2}\PB\JB^T+\VB
   \right)^{-1}
\right]   \nonumber\\
=\left(\JB^T\WB\JB+\lambda^2\RB\right)^{-1}
    \left(
       \JB^T\WB\JB+\lambda^2\RB
   \right)
    \left(
       \frac{1}{\lambda^2}\PB\JB^T
   \right)
    \left(
       \JB\frac{1}{\lambda^2}\PB\JB^T+\VB
   \right)^{-1}    \nonumber\\
=\PB\JB^T
   ...
   \right)^{-1}
\end{eqnarray}
where $\PB=\RB^{-1}=\frac{1}{\sigma_x^2}\SG_x$
  and $\VB=\WB^{-1}=\frac{1}{\sigma_n^2}\SG_n$.
In practice,
$\PB$ and $\VB$ are modelled directly from the system covariances,
rather than the inverse of $\RB$ and $\WB$.  Using
(\ref{opti_sol}), the size of the term in the inverse is reduced
to $n_M\times n_M$. This is especially significant for 3D EIT
models and for the temporal inverse which we introduce below.
...
\begin{figure}[bhtp]
\begin{center}
  \includegraphics[width= 0.2 \textwidth, bb=0 0 280 110]
...
{\em Int.J.Imaging Syst.Technol.} {\bf 2} 66--75
