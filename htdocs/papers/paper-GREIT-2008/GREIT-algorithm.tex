...
For each desired image, $\xT^{(k)}$, there is a central
circular zone centred at the target position
where the amplitude is required to be
flat to meet the AM and PE performance metrics.
There is another circular zeno outside the boundary
...
decreases from one the specified amplitude to zero.
...
\begin{figure}[bhtp]
\begin{center}
\includegraphics[width=0.8\textwidth, bb=0.0in 2.7in 10in 7.0in,page=1]{figures/fig-weighting.pdf}
...
\begin{itemize}
\item
{\em Electrode size and placement:}
...
publications explaining this?)
 ...
 For electrode
placement in adults, a mid-thoracic electrode
array 

Regarding electrode positioning to record the lung, we published (Yang
F, Patterson R. The contribution of the lungs to thoracic impedance
measurements: a simulation study based on a high resolution finite
difference model.  Physiol. Meas.  28:s153-s163, 2007) a paper using
our high resolution FD model, which showed that the lung's impedance
contribution to the total impedance vs level follows reasonably close
to the lung volume contribution slice by slice.   This suggest
therefore that the entire lung is measured from a mid-thoracic
electrode array.
 ...
 there are two
main strategies. 
One practice is to place
electrodes at a defined  nipple line, although this works
poorly for female and larger male patients. 
The other 

With regards to positioning, we have placed the electrodes a few
millimeters (in babies) or centimeters below the nipple line. Placing
electrodes too low may result in a lot of mediastinum in the center of
the EIT image.
