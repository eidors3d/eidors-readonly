...
linearized approximations have prooved to be very useful.
...
stable and better understood images in which the effects
...
maps measurements $\yY$ to a reconstructed image $\xH$,
\begin{equation} 
\label{reconst_eqn}
   \xH = \RB \yB
\end{equation} 
...
Sheffield EIT system has seen numerous 
...
were developed to improve the backprojection algorithm,
(eg Barber and Brown, 1988). Such filtering strategies
...
For this paper, it appears that the exact formulation
...
reconstructed images, $\xB_k$, and formed matrices
...
``this matrix is copyright DC Barber and BH Brown at
  ...
  applications require a licence from the University of Sheffield.''
...
\begin{equation}
\label{GREIT_norm}
\epsilon^2 = \sum_k \| \xT^{(k)} - \RB \yB^{(k)} \|_{\WB^2^{(k)}}^2
    = \sum_k \| \WB^2^{(k)} (\xT^{(k)} - \RB \yB^{(k)} ) \|^2
\end{equation}
...
-2 \sum_k \xH^{(k)}^T \WB^2^{(k)} [\TG]_{i,j} \yB^{(k)}
+2 \sum_k \yB^{(k)}^T [\TG]_{i,j} \WB^2^{(k)} \RB \yB_k
= 0
\end{equation}
...
\\
 ...
 &=&
  \sum_l [\RB]_{i,l} \sum_k [\yB^{(k)}]_l [\wB^{(k)}]_i^2 [\yT^{(k)}]_j 
\end{eqnarray}

This expression may be represented as a matrix equation
by defining matrices $\AB$ and $\BB$ where
\begin{equation}
\AB = \sum_k (\WG^{(k)})^2 \xT^{(k)} (\yB^{(k)})^T 
\end{equation}
and $\BB$ is the vertical concatenation of $n_N$
($n_M \times n_M$) block matrices $\BB^{(k)}$, where
\begin{equation}
\BB_l = \sum_k [\wG^{(k)}]_l^2 \yB^{(k)} (\yB^{(k)})^T 
\end{equation}
Based on these expressions, $\RB_{GR}$ is
\begin{equation}
