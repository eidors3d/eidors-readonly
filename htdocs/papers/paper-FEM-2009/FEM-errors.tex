...
\begin{figure}[tbh]
\begin{center}
 \includegraphics[width= 0.8\textwidth]{figures/fig_example.eps}
\caption{ \label{fig:FemExample}
\small
Simulation FEMs and simulated conductivity target positions
in blue circle. An inner region surrounding a target is shown.
...
Voltages are simulated from each FEM. In each case
...
\begin{equation}
   [\TB]_{m,n} = \frac{1}{n} \left(
        \frac{\sigma_H - \sigma_I} {\sigma_H + \sigma_I}
                             \right) e^{\ii(m-n)\alpha}
    \sum_{p=1}^{min(m,n)}
